...
class TestComplexSlice(CommonTFLayerTest):
    ...
    def create_complex_slice_net(self, input_shape, begin, size):
        tf.compat.v1.reset_default_graph()
        with tf.compat.v1.Session() as sess:
    ...
    test_data = [
        ...
        # Case of Edge
        ## Case 1: When use tensorflow's empty tensor (size == 0)
    ...
    @pytest.mark.precommit
    @pytest.mark.nightly
    def test_complex_slice(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(
